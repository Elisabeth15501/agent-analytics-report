...
| Phase 5 | P2-3（任务分类增强） | ✅ 完成 | 待提交 |
| Phase 6 | P2-2（定价自动化） | ✅ 完成 | 待提交 |
...
| ~~P2-3~~ | ~~任务分类增强~~ | ✅ 完成 | `classify_task` 重写为加权评分（信号密度 + 词边界 + 置信度）；规则外置 `task_rules.json` 可编辑；可选 LLM 分类器 opt-in（自备端点，默认离线）。**附带修复 P0：v1.3.0 拆分漏导入致 WorkBuddy 采集 NameError** |
| ~~P2-2~~ | ~~定价数据自动更新机制~~ | ✅ 完成 | `fetch_pricing.py`：自备源拉取 → 校验（schema/倍率）→ 候选 + diff 报告 → `--apply` 备份落盘；`--check --stale-days` CI 过期检查；`FETCH_PRICING_ROOT` 测试缝零接触真实文件。**附带修复：拒绝条目双重收录、default-arg 绑定绕过路径覆盖** |

### P2-3 实施说明（原计划：LLM 分类）

原计划为「升级为基于大语言模型的分类」，实施时改为**加权启发式为主 + LLM 可选增强**的混合方案，理由：

1. 纯 LLM 分类需默认调用第三方 API，与技能「离线自治、零强制依赖」的发布承诺和隐私口径冲突（会话文本属用户隐私）；
2. 生成「成本报告」本身先花一笔 LLM 费用，逻辑上自相矛盾；
3. 旧启发式的真实缺陷是**算法问题**（首匹配即返回、无词边界、无信号密度），加权评分已解决主要误判来源，实测 `fix`→`prefix`、多信号抢占等 case 全部修复；
4. LLM 保留为 opt-in（自备本地/自有端点），失败自动回退，满足愿意为此付费的高级用户。

---

## 6. 下一步

**P0-P2 全部完成**，TRACE 评测整改闭环。后续可选项：

- 报告端消费 `_task_confidence`：在任务类型表格标注低置信度行（如 ⚠️）
- `fetch_pricing.py` 接入 CI 定时跑 `--check`，过期自动开 issue
- Claude Code 适配器增强（需等 Claude Code 落盘 Skills/自动化数据）
